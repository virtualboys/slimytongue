...
{
	private List<Vector3> vertexEdit = null;
	private List<Vector3> normalEdit = null;
	...
	protected void BeginMeshEdit(Mesh mesh) {
		if (vertexEdit == null) {
			vertexEdit = new List<Vector3> (mesh.vertices);
			normalEdit = new List<Vector3> (mesh.normals);
		} else {
			vertexEdit.Clear ();
			normalEdit.Clear ();
		}
	...
	protected void EndMeshEdit(Mesh mesh) {
		//mesh.vertices = vertexEdit.ToArray ();
		//mesh.normals = normalEdit.ToArray ();
	}

	protected void RecomputeMeshProperties(Mesh mesh) {
		MeshFilter filter = GetComponent<MeshFilter>();
	...
	/// <param name="radius">The radius of the ring.</param>
	/// <param name="v">The V coordinate for this ring.</param>
	/// <param name="ringNum">Index of this ring in the meshbuilder so we can update the right vertices</param>
	/// <param name="rotation">A rotation value to be applied to the whole ring.</param>
	protected void UpdateRing(Mesh mesh, int segmentCount, Vector3 centre, float radius, int ringNum, Quaternion rotation)
	{
		int baseInd = ringNum * (segmentCount + 1);
		float angleInc = (Mathf.PI * 2.0f) / segmentCount;
		...
		for (int i = 0; i <= segmentCount; i++)
		{
			...
			vertexEdit [baseInd + i] = (centre + unitPosition * radius);
			normalEdit [baseInd + i] = (unitPosition);
			//meshBuilder.UVs [baseInd + i] (new Vector2 ((float)i / segmentCount, v));
